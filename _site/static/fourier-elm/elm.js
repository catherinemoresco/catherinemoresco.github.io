...
Elm.FourierSeries.make = function (_elm) {
   ...
   var unsafeToInt = function (x) {
      ...
      if (_p0.ctor === "Ok") {
            ...
            return _U.crashCase("FourierSeries",{start: {line: 63,column: 17},end: {line: 65,column: 58}},_p0)("unsafeToInt called on a bad string");
         }
   };
   var mailbox = $Signal.mailbox("2");
   var graphStyle = function (x) {
      return _U.update($Graphing.defaultGraph,
      {width: 800,height: 300,yInterval: {ctor: "_Tuple2",_0: -1.5,_1: 1.5},xInterval: {ctor: "_Tuple2",_0: 0,_1: 4},margin: 15,axisColor: "#fff"});
   };
   ...
   var view = function (v) {
      var event = A3($Html$Events.on,"change",$Html$Events.targetValue,$Signal.message(mailbox.address));
      var x = v;
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Graphing.graph,
              $List.concat(_U.list([A3($List.map2,
                                   ...
                                   A2($List.repeat,$Basics.round(x),_U.update($Graphing.defaultPlot,{strokeColor: "#d5d5d5"})))
   ...
   var main = A2($Signal.map,view,A2($Signal.map,function (_p2) {    return $Basics.toFloat(unsafeToInt(_p2));},mailbox.signal));
